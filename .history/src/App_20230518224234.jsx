...
const MintNFT = () => {
  ...
  return (
    <div className="container">
      {networkError && <p className="warning">Please switch to the correct network.</p>}
      <div className="main">
        <div id="img-container">
          <img src="src/assets/Spring2023.png" alt="NFT" />
        </div>
        <button onClick={checkWallet}>{userAccount != "" ? "Connected" : "Click to Connect"}</button>
        <p>{`Wallet Address: ${userAccount}`}</p>
        <input onChange={e => setUrl(e.target.value)} placeholder="Asset URL" />
        <button onClick={() => mintNft(url)}>Mint NFT</button> {/* Added url parameter */}
